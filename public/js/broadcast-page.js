...
document.addEventListener('DOMContentLoaded', async () => {
  const csrfInput = document.getElementById('csrfTokenBroadcast');
  ...
  // 1) Carregar token CSRF e colocar no hidden
  try {
    ...
    if (csrfInput && data.csrfToken) {
      ...
      console.log('🔑 CSRF broadcast:', data.csrfToken);
    } else {
      console.error('CSRF token não recebido para broadcast.');
    }
  } catch (err) {
    console.error('Erro ao carregar CSRF para broadcast:', err);
  }

  // 2) Lógica atual de contatos
  ...
  // 3) Carregar campanhas salvas
  fetch('/api/broadcast', { credentials: 'same-origin' })
    .then(res => res.json())
    .then(broadcasts => {
  ...
  // 4) Preencher form ao selecionar campanha
  savedSelect.addEventListener('change', () => {
    ...
    if (selected && selected.value) {
      ...
      form.reset();
      idsInput.value = contactIds.join(',');
      if (broadcastIdInput) broadcastIdInput.value = '';
